...
def fl_train(train_sets, fl_models, fl_optimizers, params):
    ...
    for param_i in range(len(params[0])):
        ...
        new_param= torch.div(fl_param,CLI_NUM, rounding_mode='trunc')
...
if __name__ == "__main__":
    ...
    model.to(device)
    print(model)
    #criterion=FocaLoss(device, class_num=2, alpha=None, gamma=2, size_average=True)
    ...
    train_sets=divide_trainset_to_client(train_data,CLI_NUM,BATCH_SIZE)
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
