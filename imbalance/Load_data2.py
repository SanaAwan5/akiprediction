...
class Import_Data:
    def __init__(self,file_name):
        ...
        #one_hot = pd.get_dummies(self.df['label'], prefix = 'label', drop_first = True)
        self.y = pd.get_dummies(self.df['label'], prefix = 'label', drop_first = True)
        #self.df = self.df.drop('label', axis = 1)
        #self.df = self.df.join(one_hot)
        #self.y = self.df.pop('label')
        #self.y = df[['label_0','label_1']].copy()
        #self.df = self.df.drop(['label_0','label_1'], axis =1)
