...
def fl_train(train_sets, fl_models, fl_optimizers, params):
    ...
    for k in range(len(train_sets)):
        print ('length trainsets[k]', len(trainsets[k]))
        #local_training_data= []
        #count_per_batch_ones=0
        #count_per_batch_zeros=0
        X_c = []
        Y_c = []
        for batch_idx,(data,target) in enumerate(train_sets[k]):
            #torch.utils.data.Dataset(torch.FloatTensor(data),torch.target)local_training_data.append(data,target)
            X_c.append(data)
            Y_c.append(target)	
            """samples_per_participant_per_batch = defaultdict(list)
            fl_optimizers[k].zero_grad()
            data, target = data.to(device), target.to(device)
            output = fl_models[k](data)
            count_per_batch_ones = torch.count_nonzero(target).tolist()
            count_per_batch_zeros = BATCH_SIZE - count_per_batch_ones
            samples_per_participant_per_batch[k].extend([count_per_batch_ones])
            samples_per_participant_per_batch[k].extend([count_per_batch_zeros])
            #print("samples_per_participant_per_batch", samples_per_participant_per_batch)
          
            criterion=CB_loss(k,CLI_NUM,device,BATCH_SIZE,samples_per_cls,samples_per_participant_per_batch, no_of_classes,loss_type, beta, gamma)
            loss = criterion(output, target)
            loss.backward()
            fl_optimizers[k].step()"""
        print ("List append before reshape",X_c.shape)
        X_c = torch.Tensor(X_c).reshape(X_c.shape[0]*X_c.shape[1],-1)
        Y_c = torch.Tensor(Y_c).reshape(Y_c.shape[0]*Y_c.shape[1],-1)
        print ("After Reshape",X_c.shape)
        #print('length local_training_data',len(local_training_data))
        train_loader= DataLoader(local_training_data,sampler=ImbalancedDatasetSampler(local_training_data),batch_size=BATCH_SIZE)
...
if __name__ == "__main__":
    ...
    X_train, X_test, y_train1, y_test = train_test_split(Data.X, Data.y, test_size=0.33)
    ...
    train_data1 = TrainData(torch.FloatTensor(X_train),torch.FloatTensor(y_train1.values))
