...
def fl_train(train_sets, fl_models, fl_optimizers, params, server_params):
    if (type(server_params) == 'crypten.cryptensor'):
        server_model = server_model.get_plain_text()
    new_params = list()
    ...
    print('train_samples_per_participant', train_samples)
 ...
 # get model difference
        ...
        if opt == 2:
            for server_control, control, new_control, delta in zip(server_controls, controls[k], new_controls[k],
                                                                   delta_model[k]):
    ...
    for param_i in range(len(params[0])):
        ...
        for remote_index in range(CLI_NUM):
            ...
            print("time taken to encrypt local updates", end - start)
        ...
        end1 = time.time()
        print("time taken for encrypted aggregation", end1 - start1)
    ...
    with torch.no_grad():
        ...
        #server_model = crypten.cryptensor(server_model.clone().detach()))
        #server_model = fl_param/ CLI_NUM
        #for k in range(1,CLI_NUM):
            #delta_model = fl_models[k].parameters()-fl_models[0].parameters()
        #update server_controls after every iteration as a sum of client controls (test to see if results are further improved, otherwise ignore --yes, improved :))
        control_params = list()
        for k in range(len(train_sets)):
