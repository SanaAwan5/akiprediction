...
from optimizers.fedoptimizer import *
import math
...
def clone_model_paramenter(param, clone_param):
...
def get_grads(model, grads,train_loader1):
        optimizer.zero_grad()
        ...
        for batch_idx, (x, y) in enumerate (train_loader1):
            ...
            loss.backward()
        clone_model_paramenter(model.parameters(), grads)
...
def set_grads(model,new_grads):
        if isinstance(new_grads, nn.Parameter):
            for model_grad, new_grad in zip(model.parameters(), new_grads):
                model_grad.data = new_grad.data
        elif isinstance(new_grads, list):
            for idx, model_grad in enumerate(model.parameters()):
...
def define_network(cli_num,lr_,weight_decay,input_shape):
    ...
    for i in range(cli_num):
        ...
        createVar[opti_name] = SCAFFOLDOptimizer(
            locals()[model_name].parameters(),
            lr=lr_,weight_decay=weight_decay)
        #scheduler_name = 'scheduler_' + k
        #createVar[scheduler_name] = StepLR(locals()[opti_name], step_size=8, gamma=0.1)
         
        models.append(locals()[model_name])
        params.append(list(locals()[model_name].parameters()))
        optimizers.append(locals()[opti_name])
        #schedulers.append(locals()[scheduler_name])
    createVar['server_params'] = [torch.zeros_like(p.data) for p in models[0].parameters() if p.requires_grad]
    #server_params.append(list(server()[model_name].parameters()))
    return models, optimizers, schedulers, params, server_params
...
def fl_train(train_sets, fl_models, fl_optimizers, params, server_params):
    new_params = list()
    local_training_datasets = []
    delta_model= list()
    #server_controls = [torch.zeros_like(p.data) for p in fl_models[0].parameters() if p.requires_grad]
    delta_controls = list()
    controls = list()
    controls_old = list()
    new_controls = list()
    ...
    for k in range(len(train_sets)):
        ...
        Y_c = None
        epoch_acc = 0
        #delta_model.append([torch.zeros_like(p.data) for p in fl_models[k].parameters() if p.requires_grad])
        #delta_controls.append([torch.zeros_like(p.data) for p in fl_models[k].parameters() if p.requires_grad])
        #controls.append([torch.zeros_like(p.data) for p in fl_models[k].parameters() if p.requires_grad])
        #new_controls.append([torch.zeros_like(p.data) for p in fl_models[k].parameters() if p.requires_grad])
        ...
        train_samples[k].extend([count_zeros + count_ones])
    print('train_samples_per_participant', train_samples,flush=True)
    ...
    for k in range(len(train_sets)):
        ...
        #delta_model=[torch.zeros_like(p.data) for p in fl_models[k].parameters() if p.requires_grad])
        ...
        delta_controls.append([torch.zeros_like(p.data) for p in fl_models[k].parameters() if p.requires_grad])
        controls.append([torch.zeros_like(p.data) for p in fl_models[k].parameters() if p.requires_grad])
        controls_old.append([torch.zeros_like(p.data) for p in fl_models[k].parameters() if p.requires_grad])
        new_controls.append([torch.zeros_like(p.data) for p in fl_models[k].parameters() if p.requires_grad])
        
        fl_models[k].train()
        grads = [torch.zeros_like(p.data) for p in fl_models[k].parameters() if p.requires_grad]
        grads=get_grads(fl_models[k],grads, train_sets[k])
        #print('grads', grads)
        local_epochs = 2
        for batch_idx,(data,target) in enumerate(train_sets[k]):
            ...
            loss.backward()
            fl_optimizers[k].step(server_controls,controls[k])
                #fl_schedulers[k].step()

 # get model difference
        for local, server, delta in zip(fl_models[k].parameters(),server_params, delta_model[k]):
            delta.data = local.data.detach() - server.data.detach()
            #print('delta.data',delta.data)
        # get client new controls
        
        opt = 2
        if opt == 1:
            for new_control, grad in zip(new_controls[k], grads):
                new_control.data = grad.grad
        if opt == 2:
            for server_control, control, new_control, delta in zip(server_controls, controls[k], new_controls[k],delta_model[k]):
                a = 1 / (math.ceil(train_samples[k][0] / BATCH_SIZE) * LEARNING_RATE)# or use  a = 1 / (math.ceil(len(train_sets[k]) / BATCH_SIZE) * LEARNING_RATE)
                new_control.data = control.data - server_control.data - delta.data * a

        # get controls differences
        for control, new_control, delta in zip(controls[k],new_controls[k], delta_controls[k]):
    ...
    for param_i in range(len(params[0])):
        ...
        end1=time.time()
        print("time taken to aggregate unencrypted updates", end1 - start1)
    with torch.no_grad():
        ...
        for param_index in range (len(server_params)):
            server_params[param_index].set_(new_params[param_index])
        #server_params = fl_param/ CLI_NUM
        #for k in range(1,CLI_NUM):
            #delta_model = fl_models[k].parameters()-fl_models[0].parameters()


        #update server_controls after every iteration as a sum of client controls (test to see if results are further improved, otherwise ignore)
        control_params = list()
        for k in range(len(train_sets)):
            for control in zip(controls[k]):
                #clone_controls = controls[k]
                control_params.append(controls[k])
        sign = 0
        for i in control_params:
            if sign == 0:
                control_param = i
                sign = 1
            else:
                control_param = control_param + i

        #new_param = (fl_param / CLI_NUM).get_plain_text()
        server_controls1 = [controll/CLI_NUM for controll in control_param]  
        server_controls=server_controls1
...
if __name__ == "__main__":
    ...
    X_train, X_test, y_train1, y_test1 = train_test_split(Data.X, Data.y, train_size=13000,random_state = 42)# train_size depends on each year's dataset size
    ...
    fl_models, fl_optimizers, fl_schedulers, params, server_params = define_network(
    cli_num=CLI_NUM, lr_=LEARNING_RATE,weight_decay=WEIGHT_DECAY,input_shape = X_train.shape[1])
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    #server_model = [torch.zeros_like(p.data) for p in fl_models[0].parameters() if p.requires_grad]

    model = BinaryClassification(X_train.shape[1])
    layers = [model.layer_1,model.layer_2,model.layer_3,model.layer_4,model.layer_out,model.batchnorm1,model.batchnorm2,model.batchnorm3, model.batchnorm4]
    weights = [{'params': layer.weight} for layer in layers]
    biases = [{'params': layer.bias, 'lr': 2 * LEARNING_RATE} for layer in layers]
    param_groups = [None] * (len(weights) + len(biases))
    param_groups[::2] = weights
    param_groups[1::2] = biases
    optimizer = SCAFFOLDOptimizer(model.parameters(), lr=LEARNING_RATE, weight_decay=WEIGHT_DECAY)
    ...
    for i in range(EPOCHS):
        ...
        fl_models = fl_train(train_sets, fl_models, fl_optimizers, params,server_params)
        if (i%10==0):
